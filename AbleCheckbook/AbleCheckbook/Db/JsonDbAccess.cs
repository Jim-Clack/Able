...
namespace AbleCheckbook.Db
{
    ...
    public class JsonDbAccess : IDbAccess
    {
        ...
        public MemorizedPayeeIterator MemorizedPayeeIterator
        {
            ...
            {
                _errorMessage = "";
                MemorizedPayeeIterator iterator = null;
                JsonDbAccess.Mutex.WaitOne();
                try
                {
                    iterator = new MemorizedPayeeIterator(_dbHeader.MemorizedPayees.GetEnumerator());
                }
                catch (Exception ex)
                {
                    _errorMessage = ex.Message;
                    throw new AppException("Error in MemorizedPayeeIterator()", ex, ExceptionHandling.NoSaveCleanupContinue);
                }
                finally
                {
                    JsonDbAccess.Mutex.ReleaseMutex();
        ...
        public bool InsertEntry(MemorizedPayee entry)
        {
            ...
            try
            {
                _dbHeader.MemorizedPayees.Add(entry.UniqueKey(), entry);
                if (_undoTracker != null)
                {
                    // No Undo for MemorizedPayee: _undoTracker.TrackInsertion(entry);
                }
            }
            catch (Exception ex)
            {
                _errorMessage = ex.Message;
                throw new AppException("Error in InsertEntry(MemorizedPayee)", ex, ExceptionHandling.NoSaveCleanupContinue);
            }
            finally
            {
                JsonDbAccess.Mutex.ReleaseMutex();
        ...
        public MemorizedPayee GetMemorizedPayeeById(Guid id)
        {
            ...
            try
            {
                IEnumerator<KeyValuePair<string, MemorizedPayee>> enumerator = _dbHeader.MemorizedPayees.GetEnumerator();
                while (enumerator.MoveNext())
                {
                    KeyValuePair<string, MemorizedPayee> pair = enumerator.Current;
                    if (pair.Value.Id == id)
                    {
                        payee = pair.Value;
                    }
            ...
            {
                _errorMessage = ex.Message;
                throw new AppException("Error in GetMemorizedPayeeById()", ex, ExceptionHandling.NoSaveCleanupContinue);
            }
            finally
            {
                JsonDbAccess.Mutex.ReleaseMutex();
        ...
        public MemorizedPayee GetMemorizedPayeeByName(string name)
        {
            ...
            try
            {
                ...
                {
                    KeyValuePair<string, MemorizedPayee> pair = enumerator.Current;
                    if (pair.Value.Payee.ToUpper() == name)
                    {
                        payee = pair.Value;
                    }
                }
            ...
            {
                _errorMessage = ex.Message;
                throw new AppException("Error in GetMemorizedPayeeByName()", ex, ExceptionHandling.NoSaveCleanupContinue);
            }
            finally
            {
                JsonDbAccess.Mutex.ReleaseMutex();
        ...
        public bool DeleteEntry(MemorizedPayee entry)
        {
            ...
            try
            {
                int index = _dbHeader.MemorizedPayees.IndexOfKey(entry.UniqueKey());
                if (index >= 0)
                {
                    _dbHeader.MemorizedPayees.RemoveAt(index);
                    if (_undoTracker != null)
                    {
                        // No Undo for MemorizedPayee: _undoTracker.TrackDeletion(entry);
                    }
                }
            }
            catch (Exception ex)
            {
                _errorMessage = ex.Message;
                throw new AppException("Error in RemoveEntry(MemorizedPayee)", ex, ExceptionHandling.NoSaveCleanupContinue);
            }
            finally
            {
                JsonDbAccess.Mutex.ReleaseMutex();
